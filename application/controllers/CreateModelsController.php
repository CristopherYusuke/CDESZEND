...
class CreateModelsController extends Zend_Controller_Action {
	...
	public function indexAction() {
		...
		if(!is_dir($pathBase)) mkdir($pathBase);
		...
		$model = <<<EOT
<?php
class Model_DbTable_%s extends Model_DbTable_Base_%s {
...
EOT;
		...
		$modelBase = <<<EOT
<?php
abstract class Model_DbTable_Base_%s extends ZFKiller_Db_Table {
	protected \$_name = '%s';
%s
}
EOT;
			...
			if (!empty($keys)) {
				...
				foreach ($keys as $key) {
					$refs[] = sprintf($refmap_inner,
							'ref' . ++$r,
							$key['column_name'],
							'Model_DbTable_' . $toClass->filter(array('tbl' => $key['referenced_table_name'])),
